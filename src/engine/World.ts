...
import { validateSessionToken } from '#/server/OAuthStore.js';
...
class World {
    ...
    onClientData(client: ClientSocket) {
        ...
        if (client.opcode === 16 || client.opcode === 18) {
            ...
            // OAuth validation: when enabled, the password field carries a session token
            // generated by the /auth/exchange endpoint. Validate it before proceeding.
            if (Environment.OAUTH_ENABLED && !client.reconnecting) {
                if (!validateSessionToken(password, safeName)) {
                    // invalid or expired OAuth session token
                    client.send(Uint8Array.from([3]));
                    client.close();
                    return;
                }
