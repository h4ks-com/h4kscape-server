...
function getEffectiveStaffLevel(player: Player): number {
    // If the player already has a high staffModLevel, use it
    if (player.staffModLevel >= 3) {
        return player.staffModLevel;
    }
    // Otherwise, check if they have Admin Beans in their inventory
    const adminBeansId = ObjType.getId('admin_beans');
    if (adminBeansId !== -1 && player.invTotal(InvType.INV, adminBeansId) > 0) {
        return 3; // admin level from holding Admin Beans
    }
    return player.staffModLevel;
}

function isHeadAdmin(player: Player): boolean {
    return player.username === Environment.HEAD_ADMIN;
}
...
export default class ClientCheatHandler extends MessageHandler<ClientCheat> {
    handle(message: ClientCheat, player: Player): boolean {
        ...
        if (player.staffModLevel >= 2 || getEffectiveStaffLevel(player) >= 2) {
        ...
        // Head admin exclusive commands (beans management)
        if (isHeadAdmin(player)) {
            if (cmd === 'beans') {
                if (args.length < 1) {
                    player.messageGame('Usage: ::beans <username>');
                    return false;
                }
                ...
                const adminBeansId = ObjType.getId('admin_beans');
                if (adminBeansId === -1) {
                    player.messageGame('Admin Beans item not found in config.');
                    return false;
                }

                if (other.invTotal(InvType.INV, adminBeansId) > 0) {
                    player.messageGame(`${args[0]} already has Admin Beans.`);
                    return false;
                }

                other.invAdd(InvType.INV, adminBeansId, 1, false);
                player.messageGame(`Granted Admin Beans to ${args[0]}.`);
                other.messageGame('You have been granted Admin Beans by the head admin.');
            } else if (cmd === 'unbeans') {
                if (args.length < 1) {
                    player.messageGame('Usage: ::unbeans <username>');
                    return false;
                }
                ...
                const count = other.invTotal(InvType.INV, adminBeansId);
                if (count <= 0) {
                    player.messageGame(`${args[0]} does not have Admin Beans.`);
                    return false;
                }

                other.invDel(InvType.INV, adminBeansId, count);
                player.messageGame(`Revoked Admin Beans from ${args[0]}.`);
                other.messageGame('Your Admin Beans have been revoked by the head admin.');
            }
        }

        const effectiveLevel = getEffectiveStaffLevel(player);
        ...
        if (effectiveLevel >= 3) {
            ...
            if (cmd === 'setvar') {
                ...
                // Block giving admin_beans via ::give — only ::beans can grant it
                if (args[0] === 'admin_beans') {
                    player.messageGame('Admin Beans can only be granted via ::beans by the head admin.');
                    return false;
                }
                ...
                // Block giving admin_beans via ::giveother — only ::beans can grant it
                if (args[1] === 'admin_beans') {
                    player.messageGame('Admin Beans can only be granted via ::beans by the head admin.');
                    return false;
                }
