...
export default {
    ...
    /// admin
    HEAD_ADMIN: tryParseString(process.env.HEAD_ADMIN, 'valware'),

    /// oauth
    OAUTH_ENABLED: tryParseBoolean(process.env.OAUTH_ENABLED, false),
    OAUTH_LOGTO_ENDPOINT: tryParseString(process.env.OAUTH_LOGTO_ENDPOINT, ''),
    OAUTH_LOGTO_APP_ID: tryParseString(process.env.OAUTH_LOGTO_APP_ID, ''),
    OAUTH_CALLBACK_URL: tryParseString(process.env.OAUTH_CALLBACK_URL, ''),

    /// client origin (for CORS when client is on a separate domain)
    CLIENT_ORIGIN: tryParseString(process.env.CLIENT_ORIGIN, ''),
